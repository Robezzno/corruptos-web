...
const ccaaMap = {
  ...
  refresh() {
    document.querySelector('#map-canvas svg')?.setAttribute('aria-label', i18n.t('map-aria'));
      ...
      ranking.innerHTML = `
        ...
        ${sorted.map(([name, n]) => `
          <button class="map-rank-row ${this.selected === name ? 'active' : ''}" onclick="ccaaMap.select('${name.replace(/'/g, "\\'")}')">
            <span class="map-rank-name">${escapeHTML(i18n.ccaa(name))}</span>
  ...
  tooltip(el, ev) {
    ...
    const name = el.dataset.ccaa;
    const n = this.counts[name] || 0;
    tip.innerHTML = `<strong>${escapeHTML(i18n.ccaa(name))}</strong><br>${n} ${i18n.t(n === 1 ? 'map-case' : 'map-cases')}`;
